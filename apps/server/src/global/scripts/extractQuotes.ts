...
const publishGenerateSummary = async () => {
  ...
  const tags = interview.workspace.tags
    .filter((tag) => tag.autoExtract)
  ...
  console.log("extract quotes for following tags", tags);
  assert(transcript !== null);

  await interviewPubSub.publishExtractQuotes({
